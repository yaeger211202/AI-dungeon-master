...
def build_turn_prompt(player_state, conversation_history, user_input, summaries=None):
    """
    Build a complete prompt for a single game turn.

    summaries (optional): list of strings produced by the MemoryManager
    (Step 5). Each summary compresses ~6 earlier turns into 2-3 sentences,
    so we can keep long-running coherence without blowing the context window.
    """
    history_text = "\n".join(conversation_history[-6:]) or "(no prior turns)"  # last 3 turns

    summaries_block = ""
    if summaries:
        summaries_text = "\n".join(f"- {s}" for s in summaries)
        summaries_block = f"\nStory So Far (compressed memory):\n{summaries_text}\n"
